...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform`}>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center px-6 py-3 text-gray-300 hover:bg-gray-800 hover:text-white ${isActive ? 'bg-gray-800 text-white border-l-4 border-primary-500' : ''}`}
              >
